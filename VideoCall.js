...
const VideoCall = () => {
  ...
  const setupAudioAnalyser = (userId, stream) => {
    ...
    try {
      ...
      const interval = setInterval(() => {
        ...
        setMultipleVoicesDetected(peakCount > 40);
        console.log('[VoiceDetect] multipleVoicesDetected:', peakCount > 40);
  ...
  // Generate username from user data or userId
  const generateUsername = (uid) => {
    ...
    return uid ? uid.split('_')[2] || uid : 'Anonymous';
  };
  ...
  useEffect(() => {
    ...
    const initializeMedia = async () => {
      try {
        ...
        console.log('Generated user ID:', newUserId, 'Username:', newUsername);
        ...
        try {
          ...
          const newUsername = generateUsername(newUserId);
          setUsername(newUsername);
          console.log('Generated user ID (audio only):', newUserId, 'Username:', newUsername);
        } catch (audioErr) {
  ...
  useEffect(() => {
    if (!stream || !roomId || !userId || !username) return;

    console.log('Initializing WebSocket connection for user:', userId, 'username:', username);
    ...
    socket.onopen = () => {
      ...
      // Send join message with user ID and username
    ...
    socket.onmessage = (event) => {
      ...
      switch (data.action) {
        case "user-joined":
          console.log('User joined notification:', data.peer_id, data.username);
          if (data.peer_id !== userId) {
            setMessages(prev => [...prev, { 
              text: `${data.username || data.peer_id} joined the call`, 
              from: 'System',
        ...
        case "existing-users":
          ...
          if (data.users && Array.isArray(data.users)) {
            data.users.forEach(existingUser => {
              const existingUserId = existingUser.user_id || existingUser;
              if (existingUserId !== userId) {
                ...
                if (!alreadyExists) {
                  ...
                  if (peer) {
                    setPeers(prev => [...prev, { peer, userId: existingUserId, username: existingUser.username }]);
                  }
                }
              }
            });
        ...
        case "new-peer":
        case "initial-peer": {
          console.log('New peer event:', data.peer_id, data.username);
          if (data.peer_id !== userId) {
            ...
            if (!alreadyExists && stream) {
              ...
              if (peer) {
                setPeers(prev => [...prev, { peer, userId: data.peer_id, username: data.username }]);
              }
            }
          }
          break;
          ...
          if (data.from !== userId) {
            ...
            if (!existingPeer && stream) {
              ...
              if (peer) {
                ...
                setPeers(prev => [...prev, { peer, userId: data.from, username: data.username }]);
              }
            ...
            setPeers(prev => {
              ...
              if (leavingPeer) {
                ...
                setMessages(prevMessages => [...prevMessages, { 
                  text: `${leavingPeer.username || data.peer_id} left the call`, 
                  from: 'System',
                  timestamp: new Date().toLocaleTimeString(),
                  isSystem: true
                }]);
        ...
        case "emoji-reaction": {
          ...
          if (data.from !== userId) {
            // Show floating emoji for other users' reactions
  ...
  console.log('Current peers:', peers.map(p => ({ userId: p.userId, username: p.username })));
  console.log('My user ID:', userId, 'Username:', username);
    ...
    <div style={styles.container}>
      ...
      <div style={{
        ...
        <div style={styles.videoContainer}>
          ...
          <div style={styles.videoLabel}>
            <span>You ({username}) {isCreator && '(Host)'}</span>
        ...
        {/* Remote videos */}
        {peers.map(({ userId: peerUserId, username: peerUsername }) => (
          <div key={peerUserId} style={styles.videoContainer}>
            ...
            />
            <div style={styles.videoLabel}>
              <span>{peerUsername || peerUserId.split('_')[2] || peerUserId}</span>
            </div>
          </div>
        ))}
      </div>
      ...
      {process.env.NODE_ENV === 'development' && (
        <div style={styles.debugInfo}>
          ...
          <div>WebSocket: {ws.current?.readyState === WebSocket.OPEN ? 'OPEN' : 'CLOSED'}</div>
                ...
                <div key={idx} style={{
                  ...
                  <div style={styles.messageHeader}>
                    <span style={{
                      ...
                      ...(msg.isSystem ? styles.systemSender : {})
                    }}>
                      {msg.from}
                    </span>
